    ...
    onMounted(() => {
      ...
      const getChatInfo = () => {
        ...
        if (query) {
          ...
          return Promise.resolve(JSON.parse(query));
        }
      ...
      getChatInfo().then((chatInfoData) => {
        if (chatInfoData) {
          initializeChat(chatInfoData);
        } else {
          console.error('[Chat] 无法获取聊天信息');
          uni.showToast({
            title: '无法获取聊天信息，请返回重试',
            icon: 'none'
          });
        }
      });
